package ru.holofox.anicoubs.features.data.network.api.vk.builders

class VKParametersBuilder(b: Builder) {
    ...
    class Builder {
        ...
        fun isPrivate(isPrivate: Boolean) =
            apply { this.args["is_private"] = if (isPrivate) "1" else "0" }

        fun wallPost(wallPost: Boolean) =
            apply { this.args["wallpost"] = if (wallPost) "1" else "0" }
        ...
        fun privacyComment(privacyComment: String) =
            apply { this.args["privacy_comment"] = privacyComment }

        fun noComments(noComments: Boolean) =
            apply { this.args["no_comments"] = if (noComments) "1" else "0" }
        ...
        fun fromGroup(fromGroup: Boolean) =
            apply { this.args["from_group"] = if (fromGroup) "1" else "0" }
        ...
        fun muteNotification(mute: Boolean) =
            apply { this.args["mute_notifications"] = if (mute) "1" else "0" }
